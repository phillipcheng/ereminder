...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        allPossible = util.Counter()
        for p in self.legalPositions:
            npd = self.getPositionDistribution(self.setGhostPosition(gameState, p))
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            self.ppList.append(self.legalPositions[i % len(self.legalPositions)])
        #logging.debug('particle positions: %s', self.ppList)
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None :
            self.ppList = [self.getJailPosition()]
        else:
            ...
            for p in self.ppList:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                if emissionModel[trueDistance] > 0:
                    weights[p] = emissionModel[trueDistance] * bd[p]
                    totalNoneZero +=1
            if totalNoneZero==0:
                self.initializeUniformly(gameState)
            else:
                items = sorted(weights.items())
                self.ppList = util.nSample([v for k,v in items], [k for k,v in items], self.numParticles)
...
class JointParticleFilter:
    ...
    def initialize(self, gameState, legalPositions):
        ...
        self.deadGhosts = [] #store the idx of the dead ghosts
        self.ppList = None #store the list of particle positions
    ...
    def initializeParticles(self):
        ...
        for i in range(self.numGhosts):
            aList = []
            if i in self.deadGhosts:
                aList = [self.getJailPosition(i)] * self.numParticles
            else:
                for i in range(self.numParticles):
                    aList.append(self.legalPositions[i % len(self.legalPositions)])
            if self.ppList==None:
                self.ppList = aList
            else:
                self.ppList = zip(self.ppList, aList)
        #logging.debug('particle list: %s', self.ppList)
    ...
    def observeState(self, gameState):
        ...
        for i in range(self.numGhosts):
            ...
            if noisyDistance == None :
                ...
                for pp in self.ppList:
                    trueDistance = util.manhattanDistance(pp[i], pacmanPosition)
                    if emissionModel[trueDistance] > 0:
                        weights[pp[i]] = emissionModel[trueDistance] * dist[pp[i]]
                        totalNoneZero +=1
                if totalNoneZero!=0:
                    for pp in self.ppList:
                        ppl = list(pp)
