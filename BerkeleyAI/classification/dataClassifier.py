...
def getZeroCount(parent):
    n=0
    for k, v in parent.items():
        if v==0: n+=1
    return n

def getPixel(datum, x, y):
    if x==-1 and y==-1:#for all the outer region set to 0, suppose on a white paper
        return 0
    else:
        return datum.getPixel(x, y)

def getNeighbors(datum, pos):
    x,y=pos
    candlist = []
    if (x==-1 and y==-1):
        for i in range(DIGIT_DATUM_WIDTH):
            candlist.append((i, 0))
            candlist.append((i, DIGIT_DATUM_HEIGHT-1))
        for i in range(DIGIT_DATUM_HEIGHT):
            candlist.append((0, i))
            candlist.append((DIGIT_DATUM_WIDTH-1, i))
    else:
        trylist = ((x+1,y),(x-1,y),(x,y+1),(x,y-1),(x+1, y+1), (x+1, y-1), (x-1, y+1), (x-1, y-1))
        for npos in trylist:
            if npos[0]==-1 or npos[0]==DIGIT_DATUM_WIDTH or npos[1]==-1 or npos[1]==DIGIT_DATUM_HEIGHT:
                if not (-1,-1) in candlist:
                    candlist.append((-1,-1))
            else:
                candlist.append(npos)
    retlist = []
    for npos in candlist:
        if (getPixel(datum,npos[0],npos[1])==0 and getPixel(datum,pos[0],pos[1])==0 or \
                     getPixel(datum,npos[0],npos[1])>0 and getPixel(datum,pos[0],pos[1])>0):
            retlist.append(npos)
    return retlist
maxVerticalN=13
maxVerticalV=4
...
def enhancedFeatureExtractorDigit(datum):
    ...
    ## 1. the number of crosses with several vertical line
    ## 2. the of pixel in the upper or lower region
    ## 3. number of ends
    """
    ...
    global maxVerticalN
    global maxVerticalV
    if maxVerticalN<len(vectorV): maxVerticalN = len(vectorV)
    if maxVerticalV<n: maxVerticalV = n
    '''
    ...
    for y in range(DIGIT_DATUM_HEIGHT):
        ...
        global maxHorizonN
        global maxHorizonV
        if maxHorizonN<len(vectorH): maxHorizonN = len(vectorH)
        if maxHorizonV<n: maxHorizonV = n
    '''
    parent = util.Counter()
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
    ...
    while getZeroCount(parent)>0:
        ...
        while (not q.isEmpty()):
            pos = q.pop()
            parent[pos]=cluster
            nl = getNeighbors(datum, pos)
            for n in nl:
                if parent[n]==0 and (not n in q.list):
                    q.push(n)
    '''
    global maxcc
    if cluster>maxcc: maxcc=cluster
    '''
    #print("vectorV:%s, numV:%s, maxVerticalN:%d, maxVerticalV:%d" % (vectorV, numV, maxVerticalN, maxVerticalV))
    #print("vectorH:%s, maxHorizonN:%d, maxHorizonV:%d" % (vectorH, maxHorizonN, maxHorizonV))
    #print("cc:%d, maxcc:%d" % (cluster, maxcc))
    '''
    for i in range(maxHorizonN):
        for j in range(maxVerticalV+1):
            if (i>=len(vectorV)):
                if (j==0):
                    features['seqV_' + str(i) + '_' + str(j)]=1
                else:
                    features['seqV_' + str(i) + '_' + str(j)]=0
            else:
                if vectorV[i]==j:
                    features['seqV_' + str(i) + '_' + str(j)]=1
                else:
    ...
    '''
    for i in range(maxHorizonN):
        for j in range(maxHorizonV+1):
            if (i>=len(vectorH)):
                if (j==0):
                    features['seqH_' + str(i) + '_' + str(j)]=1
                else:
                    features['seqH_' + str(i) + '_' + str(j)]=0
            else:
                if vectorH[i]==j:
                    features['seqH_' + str(i) + '_' + str(j)]=1
                else:
                    features['seqH_' + str(i) + '_' + str(j)]=0
    '''
    for i in range(maxcc):
        if (cluster==i+1):
            features['cc_' + str(i)]=1
        else:
            features['cc_' + str(i)]=0
...
def enhancedPacmanFeatures(state, action):
    ...
    distance to ghost
    distance to food
    distance from ghost to capsules
    """
    
    pacPos = nextState.getPacmanPosition()
    idir = 0
    dir = nextState.getPacmanState().getDirection()
    if (dir==Directions.NORTH):
        ...
        idir=3
    elif dir == Directions.STOP:
        idir=4
    features['pDir'] = idir
    
    for i in range(nextState.getNumAgents()-1):
        gps = nextState.getGhostState(i+1)
        cd = util.manhattanDistance(gps.getPosition(), pacPos)
        features['ghostTimer' + str(i)] = gps.scaredTimer
        features['ghostD' + str(i)] = cd
    
    i=0
    for cPos in nextState.getCapsules():
        i+=1
        cd = util.manhattanDistance(pacPos, cPos)
        features['capsuleD'+str(i)] = cd
    ...
    for cPos in nextState.getCapsules():
        i+=1
        for j in range(nextState.getNumAgents()-1):
            gps = nextState.getGhostState(j+1)
            cd = util.manhattanDistance(gps.getPosition(), cPos)
            features['capsuleGhostD'+str(i)+'_'+str(j)] = cd
    
    score = nextState.getScore()
    features['score'] = score
    '''
    i=0
    for fPos in nextState.getFood().asList():
        i+=1
        fd = util.manhattanDistance(pacPos, fPos)
        features['foodD' + str(i)] = fd
    '''
...
def analysis(classifier, guesses, testLabels, testData, rawTestData, printImage):
    ...
    print "num of guesses:%d" % len(guesses)
    for i in range(len(guesses)):
        ...
        if (prediction != truth):
            ...
            print "Image: "
            print rawTestData[i]
            enhancedFeatureExtractorDigit(rawTestData[i])
