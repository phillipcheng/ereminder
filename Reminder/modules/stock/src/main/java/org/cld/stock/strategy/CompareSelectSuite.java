...
public class CompareSelectSuite {
	...
	public static SelectStrategy getHSABreakIssue(String outputFileDir){
		...
		String sb = "select stockid, close from SinaMarketFQ where dt='%s'";//BackFQ
		return new CompareSelectStrategy("hsaBreakIPO", outputFileDir, new Object[]{new String[]{sa, sb}, new String[]{"(b-a)/a"}, "a>b", "za", "desc"});
	}

	public static SelectStrategy getHSAAllTimeLow(String outputFileDir){
		String sa = "select stockid, min(close) from SinaMarketFQ where dt<='%s' group by stockid";
		String sb = "select stockid, close from SinaMarketFQ where dt='%s'";
	...
	public static SelectStrategy getHSARallyRatio(String outputFileDir){
		...
		String sc = "select stockid, min(close) from SinaMarketFQ where dt<'%s' and dt>'2015-08-18' group by stockid";//this low inc-adjusted close
		String sd = "select stockid, close from SinaMarketFQ where dt='%s'";//inc-adjusted close
		String se = "select stockid, dt, dilutedeps from (select *, row_number() over (partition by stockid order by dt desc) as rn "
				+ "from SinaFrProfitStatement where datediff('%s',dt)>0) q where rn=1";//eps
		String sf = "select stockid, name from SinaCorpInfo";//name
		String sg = "select stockid, close from SinaMarketDaily where dt='%s'";//close
	...
	public static SelectStrategy getNasdaqAllTimeLow(String thisDay, CrawlConf cconf, String outputFileDir){
		String sqlA = "select stockid, min(adjclose) from NasdaqFQHistory where dt<='%s' group by stockid";
		String sqlB = "select stockid, adjclose from NasdaqFQHistory where dt='%s'";
		String filter = "a>=b";
		return new CompareSelectStrategy("nasdaqAllTimeLow", outputFileDir, new Object[]{
				new String[]{sqlA, sqlB}, new String[]{"(b-a)/a"}, filter, "za", "desc"});
	}
	...
	public static SelectStrategy getNasdaqBreakIssue(String outputFileDir){
		...
		String sqlC = "select stockid, adjclose from NasdaqFQHistory where dt='%s'"; //ForwardFQ
		return new CompareSelectStrategy("nasdaqBreakIPO", outputFileDir, new Object[]{new String[]{sqlA, sqlB, sqlC}, new String[]{"(a*b-c)/c"}, "a*b>c", "za", "desc"});
	}
