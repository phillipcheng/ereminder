...
public class OpenCloseRaiseAvgD extends SelectStrategy {

	public static final int LOOKUP_DAYS=40;
	
	private static SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
	private StockConfig sc;
	
	public OpenCloseRaiseAvgD(){
	}
				...
				if (checkValid(oneDayBeforeSumbitCq)){
					...
					if (low>0){
						if (DateTimeUtil.DateDiff(oneDayBeforeSumbitCq.getStartTime().getTime(),thisCq.getStartTime().getTime())<=LOOKUP_DAYS){
							float value = (submitCq.getOpen()-low)/(low*days);
