...
public class ProductListAnalyze implements ListProcessInf, IProductListAnalyze {
	...
	public List<Task> readItem(WebClient wc, HtmlPage page, Category category, DomNode productSummary, 
		...
		for (TargetPrdInvoke fullOutput:targetPrdInvokeList){
			if (task.getLeafBrowseCatTask().getSplitTaskType()==SplitTaskByType.ITEM){
				...
				BrowseProductTaskConf taskTemplate = (BrowseProductTaskConf) cconf.getTaskMgr().getTask(fullOutput.getTaskName());
				BrowseProductTaskConf.browseProduct(taskTemplate, cconf, ctconf, wc, storeId, catId, fullOutput.getTaskName(), fullOutput.getInParams());
			}
		}
		return tl;
	}	
	...
	public List<Task> process(HtmlPage listPage, Date readTime,
		...
		WebClient myWC = CrawlUtil.getWebClient(cconf, task.getSkipUrls(), 
				task.getBrowseDetailTask(null).getBrowsePrdTaskType().getBaseBrowseTask().isEnableJS());//get 1st browsePrdTask enable js defintion
		...
		try {
			if (itemList != null){
				...
				for (int i=0; i<itemList.size(); i++){
					...
					List<Task> tl1= readItem(myWC, listPage, cat, itemList.get(i), null, readTime, cconf, task);
					if (tl1!=null){
						tl.addAll(tl1);
					}
				}
			...
			if (myWC!=null){
				CrawlUtil.closeWebClient(myWC);
			}
			logger.debug("new wc closed." + myWC);
		}
		return tl;
	}
}
