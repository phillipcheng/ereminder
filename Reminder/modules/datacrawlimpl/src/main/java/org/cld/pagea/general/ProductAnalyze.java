...
public class ProductAnalyze implements ProductAnalyzeInf {
	...
	private static Map<String,List<? extends DomNode>> getFirstPage(WebClient wc, HtmlPage prdPage, 
		...
		if (firstPageClicks==null || "".equals(firstPageClicks)){
			...
			if (firstPageClicks.getFinishCondition()==null){
				...
				for (ClickType clickType: firstPageClicks.getLink()){
					...
					if (value!=null && value instanceof HtmlPage){
						...
						logger.error(String.format("click stream:%s eval to %s, not a page, check toType. prdPage is:%s", 
								vt.getValue(), value, prdPage.getUrl().toExternalForm()));
						break;
					}
				}
			}else{
	...
	private HtmlPage getNextPage(WebClient wc, HtmlPage curPage, BrowseDetailType bdt, Task task) throws InterruptedException{
		if (bdt.getNextPage()!=null){
			...
			if (nextPageEle!=null || url!=null){
				...
				HtmlPageResult hpResult = HtmlUnitUtil.clickNextPageWithRetryValidate(wc, np, 
						new VerifyPageByXPath(getPageVerifyXPaths(task)), task, cconf.getMaxRetry(), false);
	...
	public void callbackReadDetails(WebClient wc, HtmlPage inpage, Product product, Task task, ParsedBrowsePrd taskDef) throws InterruptedException {	
		...
		if (product.getLastUrl()==null){
			...
			HtmlPageResult hpResult = HtmlUnitUtil.clickURLWithRetryValidateByXPaths(wc, product.getLastUrl(), 
					 getPageVerifyXPaths(task), cconf.getMaxRetry(), cconf.isCancelable());
			if (hpResult.getErrorCode()==HtmlPageResult.SUCCSS){
		...
		while (curPage!=null && !finalPage && !externalistFinished){
			...
			curPage=getNextPage(wc, curPage, bdt, task);
		}
