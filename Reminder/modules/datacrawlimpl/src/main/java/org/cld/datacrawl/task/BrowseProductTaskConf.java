...
public class BrowseProductTaskConf extends Task implements Serializable{
	 ...
	 * @param task: containing the whole parsed task definition tree
	 ...
	 */
	public static void browseProduct(Task task, CrawlConf cconf, CrawlTaskConf ctconf, WebClient wc, String storeId, 
			...
			if (internalId!=null && !"".equals(internalId)){			
				...
				if (lastProduct != null){
					...
					if (task.getBrowseDetailTask(null).getBrowsePrdTaskType().isMonitorPrice()){
						...
						thisDetailPrice = ba.readPrice(wc, startUrl, internalId, storeId, null, task);
