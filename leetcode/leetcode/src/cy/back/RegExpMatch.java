		...
		if (ls!=null){
			...
			for (int i=0;i<ls.size(); i++){
				...
				if (seg.type==0){//constant
					...
					if (seg.type==0){
						//matching [seg2,(seg1|seg2|seg3)*] seg0
						int k=idx;
						boolean pass=false;
						...
						//matching [seg2, (seg1|seg2|seg3)*] to all the rest of the chars						
				...
				}else if (seg.type==3){//a*
					if (idx>=chars.length){
						log(seg + " matches nothing, and let go.");
					}else{
						int startIdx=idx;
						if (chars[idx]==seg.c){
							//match more
							...
							while (idx<chars.length){
								if (seg.c==chars[idx]){
									...
									log(seg + " matched:" + new String(chars, startIdx, idx-startIdx));
									break;
								}
							}
							if (idx>=chars.length){
